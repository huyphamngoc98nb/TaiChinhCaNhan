...
export function validateCreateTransaction(input: CreateTransactionInput) {
  ...
  // Transfer constraint validation (replaces trg_transactions_transfer_check_ins)
...
export function validateUpdateTransaction(input: UpdateTransactionInput) {
  ...
  // Transfer constraint validation (replaces trg_transactions_transfer_check_upd)
  // Only validate if type is being changed — caller must provide to_wallet_id
  // when changing TO transfer, or clear it when changing FROM transfer.
  ...
  if (input.type !== 'transfer' && input.type !== undefined && input.to_wallet_id !== undefined && input.to_wallet_id !== null) {
    errors.push('to_wallet_id must be null for non-transfer transactions');
  }
