...
function determineSeverity(errorMessage: string, testTitle: string): 'P1' | 'P2' | 'P3' {
...
function generateReport() {
  ...
  // Traverse suites to find failed tests
          ...
          if (lastResult) {
            if (lastResult.status === 'passed') {
              ...
              if (screenshotAttachment) {
                // Make path relative to workspace or copy it
  ...
  if (failedTests.length === 0) {
    ...
    failedTests.forEach((bug, index) => {
      ...
      if (bug.screenshot) {
        // Playwright saves absolute paths, let's make a clickable local file path
